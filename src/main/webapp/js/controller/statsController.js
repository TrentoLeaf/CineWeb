/**
 * Created by Willo on 24/06/2015.
 */
(function () {
    'use strict';

    /*Io devo fare:
    * -Lista dei posti venduti per ciascuna programmazione
    * -Lista incassi per film
    * -Lista dei clienti che comprano di più
    * */

    var scheduleSoldSeatsStatsData = {
        labels: ["Eating", "Drinking", "Sleeping", "Designing", "Coding", "Cycling", "Running"],
        datasets: [
            {
                label: "My First dataset",
                fillColor: "rgba(95,255,89,0.2)",
                strokeColor: "rgba(95,255,89,1)",
                pointColor: "rgba(95,255,89,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(95,255,89,1)",
                data: [65, 59, 90, 81, 56, 55, 40]
            },
            {
                label: "My Second dataset",
                fillColor: "rgba(255,53,31,0.2)",
                strokeColor: "rgba(255,53,31,1)",
                pointColor: "rgba(255,53,31,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(255,53,31,1)",
                data: [28, 48, 40, 19, 96, 27, 100]
            }
        ]
    };

    var filmsIncomeData = {
        labels: ["Eating", "Drinking", "Sleeping", "Designing", "Coding", "Cycling", "Running"],
        datasets: [
            {
                label: "My First dataset",
                fillColor: "rgba(255,248,41,0.2)",
                strokeColor: "rgba(255,248,41,1)",
                pointColor: "rgba(255,248,41,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(255,248,41,1)",
                data: [65, 59, 90, 81, 56, 55, 40]
            },
            {
                label: "My Second dataset",
                fillColor: "rgba(151,187,205,0.2)",
                strokeColor: "rgba(151,187,205,1)",
                pointColor: "rgba(151,187,205,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(151,187,205,1)",
                data: [28, 48, 40, 19, 96, 27, 100]
            }
        ]
    };

    var topClientsData = {
        labels: ["Eating", "Drinking", "Sleeping", "Designing", "Coding", "Cycling", "Running"],
        datasets: [
            {
                label: "My First dataset",
                fillColor: "rgba(220,220,220,0.2)",
                strokeColor: "rgba(220,220,220,1)",
                pointColor: "rgba(220,220,220,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(220,220,220,1)",
                data: [65, 59, 90, 81, 56, 55, 40]
            },
            {
                label: "My Second dataset",
                fillColor: "rgba(242,54,255,0.2)",
                strokeColor: "rgba(242,54,255,1)",
                pointColor: "rgba(242,54,255,1)",
                pointStrokeColor: "#fff",
                pointHighlightFill: "#fff",
                pointHighlightStroke: "rgba(242,54,255,1)",
                data: [28, 48, 40, 19, 96, 27, 100]
            }
        ]
    };

    /*Chiedere a Sam*/
    angular.module('adminStats', ['filmsPlaysModule'])
        .controller('AdminStatsController', ['$location', 'Films', function ($location, Films) {
            console.log("Hola");
            var ctx = document.getElementById("scheduleSoldSeatsStats").getContext("2d");
            var scheduleSoldSeatsStats = new Chart(ctx).Radar(scheduleSoldSeatsStatsData, {pointDot: true});

            var ctx = document.getElementById("filmsIncome").getContext("2d");
            var filmsIncome = new Chart(ctx).Radar(filmsIncomeData, {pointDot: false});

            var ctx = document.getElementById("topClients").getContext("2d");
            var topClients = new Chart(ctx).Radar(topClientsData, {pointDot: false});

        }]);
})();
